    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['softening:softeningpoint:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['softening:softeningpoint:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['softening:softeningpoint:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['softening:softeningpoint:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="softeningpointList" @selection-change="handleSelectionChange">
...
<!--      <el-table-column label="${comment}" align="center" prop="id" />-->
      ...
      <el-table-column label="软化点温度" align="center" prop="softenP" />
      <el-table-column label="软化点平均值" align="center" prop="average" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="任务单编号" prop="taskNo">
          <el-input v-model="form.taskNo" placeholder="请输入任务单编号" />
        </el-form-item>
        ...
        <el-form-item label="试验温度/起始温度" prop="testTemp">
          <el-input v-model="form.testTemp" placeholder="请输入试验温度/起始温度" />
        </el-form-item>
        <el-form-item label="软化点时间" prop="softenTime">
          <el-input v-model="form.softenTime" placeholder="请输入软化点时间" />
        </el-form-item>
        <el-form-item label="软化点温度" prop="softenP">
          <el-input v-model="form.softenP" placeholder="请输入软化点温度" />
        </el-form-item>
        <el-form-item label="软化点平均值" prop="average">
          <el-input v-model="form.average" placeholder="请输入软化点平均值" />
        </el-form-item>
        <el-form-item label="缺省为0；0代表合格，1代表不合格" prop="result">
          <el-input v-model="form.result" placeholder="请输入缺省为0；0代表合格，1代表不合格" />
        </el-form-item>
...
<script>
import { listSofteningpoint, getSofteningpoint, delSofteningpoint, addSofteningpoint, updateSofteningpoint } from "@/api/softening/softeningpoint";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSofteningpoint(this.queryParams).then(response => {
        this.softeningpointList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSofteningpoint(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSofteningpoint(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSofteningpoint(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除软化点实验编号为"' + ids + '"的数据项？').then(function() {
        return delSofteningpoint(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('softening/softeningpoint/export', {
        ...this.queryParams
      }, `softeningpoint_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
